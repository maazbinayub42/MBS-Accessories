import { CMSContent, Product, Category, NavigationItem, MediaItem } from "@/types";

const STORAGE_KEY = "mbs_cms_content";
...
const DEFAULT_CONTENT: CMSContent = {
  ...
  navigation: [
    { id: "nav-1", label: "Home", url: "/", enabled: true, order: 1 },
    { id: "nav-2", label: "Shop", url: "/shop", enabled: true, order: 2 },
    { id: "nav-3", label: "Kufis", url: "/shop?category=kufis", enabled: true, order: 3 },
    { id: "nav-4", label: "Imamas", url: "/shop?category=imamas", enabled: true, order: 4 },
    { id: "nav-5", label: "Attars", url: "/shop?category=attars", enabled: true, order: 5 },
    { id: "nav-6", label: "Fragrances", url: "/shop?category=fragrances", enabled: true, order: 6 },
    { id: "nav-7", label: "Our Story", url: "/about", enabled: true, order: 7 },
    { id: "nav-8", label: "Contact", url: "/contact", enabled: true, order: 8 },
  ],
  homepage: {
    hero: {
...
let _content: CMSContent | null = null;

function ensureContent(): CMSContent {
  if (!_content) init();
  return _content!;
}
...
function mergeDeep(target: unknown, source: unknown): unknown {
  const output = deepClone(target) as Record<string, unknown>;
  ...
  for (const key in src) {
    if (Object.prototype.hasOwnProperty.call(src, key)) {
      if (
        ...
        output[key] = mergeDeep(output[key], src[key]);
      } else if (src[key] !== undefined) {
        output[key] = deepClone(src[key]);
      }
    }
  }
  return output;
...
function init(): CMSContent {
  try {
    if (typeof window === "undefined") {
      _content = deepClone(DEFAULT_CONTENT);
      return _content;
    }
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored) {
      const parsed = JSON.parse(stored);
      _content = mergeDeep(DEFAULT_CONTENT, parsed) as CMSContent;
    } else {
      _content = deepClone(DEFAULT_CONTENT);
...
function get(path?: string): unknown {
  ...
  if (!path) return deepClone(content);
  const value = getNestedValue(
    content as unknown as Record<string, unknown>,
    path
  );
  if (typeof value === "object" && value !== null) return deepClone(value);
  return value;
}
...
function getAll(): CMSContent {
  return deepClone(ensureContent());
}
...
function getProducts(): Product[] {
  return deepClone(ensureContent().products);
}
...
function addProduct(product: Partial<Product>): Product {
  const content = ensureContent();
  const newProduct = deepClone(product) as Product;
  if (!newProduct.id) newProduct.id = generateId("prod");
  if (!newProduct.slug) {
...
function updateProduct(id: string, data: Partial<Product>): Product | null {
  const content = ensureContent();
  const index = content.products.findIndex((p) => p.id === id);
  if (index === -1) return null;
  content.products[index] = mergeDeep(content.products[index], data) as Product;
  content.products[index].id = id;
  save();
  return deepClone(content.products[index]);
}
...
function getCategories(): Category[] {
  return deepClone(ensureContent().categories);
}

function addCategory(cat: Partial<Category>): Category {
  const content = ensureContent();
  const newCat = deepClone(cat) as Category;
  if (!newCat.id) newCat.id = generateId("cat");
  if (!newCat.slug) {
  ...
  content.categories.push(newCat);
  save();
...
function updateCategory(id: string, data: Partial<Category>): Category | null {
  const content = ensureContent();
  const index = content.categories.findIndex((c) => c.id === id);
  if (index === -1) return null;
  content.categories[index] = mergeDeep(content.categories[index], data) as Category;
  content.categories[index].id = id;
  save();
  return deepClone(content.categories[index]);
}
...
function getNavigation(): NavigationItem[] {
  const nav = deepClone(ensureContent().navigation);
  nav.sort((a, b) => (a.order || 0) - (b.order || 0));
  return nav;
}

function addNavItem(item: Partial<NavigationItem>): NavigationItem {
  const content = ensureContent();
  const newItem = deepClone(item) as NavigationItem;
  if (!newItem.id) newItem.id = generateId("nav");
  if (newItem.order === undefined) {
    const maxOrder = content.navigation.reduce(
      (max, n) => Math.max(max, n.order || 0),
      0
    );
    newItem.order = maxOrder + 1;
  }
  content.navigation.push(newItem);
  save();
...
function updateNavItem(
  ...
  const content = ensureContent();
  const index = content.navigation.findIndex((n) => n.id === id);
  if (index === -1) return null;
  content.navigation[index] = mergeDeep(content.navigation[index], data) as NavigationItem;
  content.navigation[index].id = id;
  save();
  return deepClone(content.navigation[index]);
}

function deleteNavItem(id: string): boolean {
  const content = ensureContent();
  const index = content.navigation.findIndex((n) => n.id === id);
  if (index === -1) return false;
  content.navigation.splice(index, 1);
  save();
  return true;
}

function getMedia(): MediaItem[] {
  return deepClone(ensureContent().media);
}

function addMedia(item: Partial<MediaItem>): MediaItem {
  const content = ensureContent();
  const newItem = deepClone(item) as MediaItem;
  if (!newItem.id) newItem.id = generateId("media");
  if (!newItem.createdAt) newItem.createdAt = new Date().toISOString();
  content.media.push(newItem);
  save();
  return deepClone(newItem);
}
...
function deleteMedia(id: string): boolean {
  const content = ensureContent();
  const index = content.media.findIndex((m) => m.id === id);
  if (index === -1) return false;
